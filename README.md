...
### 作成背景
- インターン先のシフト確認がGoogleスプレッドシート運用で、読み込みが遅かったり、他の方のシフトが記載されていたりして、確認が手間だった
...
### スナップショット
![入力例](<スクリーンショット 2026-03-12 020831.png>)
![確認モーダル](<スクリーンショット 2026-03-12 020851.png>)
![給与情報](<スクリーンショット 2026-03-12 021040.png>)
![カレンダー](<スクリーンショット 2026-03-12 020946-1.png>)
...
## 開発者向け
記事書きました:[https://qiita.com/ars0611/items/c163d025c586e89acaa7](https://qiita.com/ars0611/items/c163d025c586e89acaa7)
### 技術スタック
- Framework: WXT, React
- Language: TypeScript
...
```text
src/
├─ entrypoints/                  # 拡張機能の入口（popup/content/background）
│  ├─ popup/                     # ポップアップ画面の起点
│  │  ├─ App.tsx
│  │  └─ main.tsx
│  ├─ background.ts              # バックグラウンド処理
│  └─ content.tsx                # スプレッドシートタブで動くコンテンツスクリプト
├─ ui/
│  ├─ sections/                  # 画面の機能単位（シート読込/設定/モーダル等）
│  ├─ components/                # UIコンポーネント（Button/Input など）
│  └─ cssModules/                # コンポーネント専用スタイル
├─ API/
│  ├─ services/                  # APIを叩く処理
│  └─ clients/                   # Chrome runtime によるpopupとbackgroundの通信処理
├─ utils/                        # 純粋関数中心のユーティリティ
├─ types/                        # 型定義（message/sheet/calendar 等）
```

よく触る実装ポイント:
- 画面全体の構成: `src/entrypoints/popup/App.tsx`
- 入力系 UI（例: `EditableNumberRow`）: `src/ui/components/Input.tsx`
- セクションごとの処理: `src/ui/sections/*.tsx`
- Sheets 読込ロジック: `src/API/services/loadSheetService.ts`
- Calendar 反映ロジック: `src/API/services/importToCalendarService.ts`
...
4. 拡張機能IDをGoogle CloudのアイテムIDに入力
